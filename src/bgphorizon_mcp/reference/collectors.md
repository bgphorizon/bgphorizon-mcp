...
usually a measurement artifact**, not a global routing event. That is the reason every
...
- `effective_vantage_points`: inverse-Simpson index of collector shares; a low
  number (near 1) means the view rests on one collector even if many
  are technically present.
- `unique_collectors`: how many collectors saw the prefix/ASN at all.

A prefix seen by 24 collectors with an even share is a broad global view; one
